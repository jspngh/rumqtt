...
use rumqtt_bytes::{
    Disconnect, Filter, PubAck, Publish, PublishProperties, Subscribe, SubscribeProperties,
    Unsubscribe, UnsubscribeProperties,
};

use crate::topic::{valid_filter, valid_topic};
use crate::{EventLoop, MqttOptions, Packet, QoS};
...
#[derive(Clone, Debug)]
pub struct AsyncClient<P: Protocol<Item = Packet>> {
    request_tx: Sender<Packet>,
    protocol: std::marker::PhantomData<P>,
}

/// Asynchronous client for MQTT 3.1.1 (protocol version 4)
pub type AsyncClientV4 = AsyncClient<rumqtt_bytes::V4>;
/// Event loop for MQTT 3.1.1 (protocol version 4)
pub type EventLoopV4 = EventLoop<rumqtt_bytes::V4>;

/// Asynchronous client for MQTT 5.0 (protocol version 5)
pub type AsyncClientV5 = AsyncClient<rumqtt_bytes::V5>;
/// Event loop for MQTT 5.0 (protocol version 5)
pub type EventLoopV5 = EventLoop<rumqtt_bytes::V5>;

impl AsyncClientV4 {
    /// Create a new `AsyncClient` for MQTT 3.1.1 communication
    ///
    /// `cap` specifies the capacity of the bounded async channel.
    pub fn new(options: MqttOptions, cap: usize) -> (AsyncClientV4, EventLoopV4) {
        let (eventloop, request_tx) = EventLoop::new(options, cap);

        let client = AsyncClientV4 {
            request_tx,
...
impl AsyncClientV5 {
    ...
    ///
    /// `cap` specifies the capacity of the bounded async channel.
    pub fn new_v5(options: MqttOptions, cap: usize) -> (AsyncClientV5, EventLoopV5) {
        let (eventloop, request_tx) = EventLoop::new(options, cap);

        let client = AsyncClientV5 {
            request_tx,
            protocol: std::marker::PhantomData,
        };
...
fn get_ack_req(publish: &Publish) -> Option<Packet> {
    let ack = match publish.qos {
        QoS::AtLeastOnce | QoS::ExactlyOnce => Packet::PubAck(PubAck::new(publish.pkid)),
        _ => return None,
    };
    Some(ack)
}
...
mod test {
    ...
    fn calling_iter_twice_on_connection_shouldnt_panic() {
        ...
        let options = OptionsBuilder::new_tcp("localhost", 1883)
            .client_id("test-1")
    ...
    fn should_be_able_to_build_test_client_from_channel() {
        ...
        let client = ClientV5::from_sender(tx);
        client
