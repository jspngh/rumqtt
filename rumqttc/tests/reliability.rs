...
async fn connection_should_timeout_on_time() {
    ...
    let options = OptionsBuilder::new_tcp("127.0.0.1", 1880)
        .client_id("dummy")
...
fn test_invalid_keep_alive_value() {
    OptionsBuilder::new_tcp("127.0.0.1", 1885).keep_alive(Duration::from_millis(10));
}

#[test]
fn test_zero_keep_alive_values() {
    OptionsBuilder::new_tcp("127.0.0.1", 1885).keep_alive(Duration::ZERO);
}

#[test]
fn test_valid_keep_alive_values() {
    OptionsBuilder::new_tcp("127.0.0.1", 1885).keep_alive(Duration::from_secs(1));
}
...
async fn idle_connection_triggers_pings_on_time() {
    ...
    let options = OptionsBuilder::new_tcp("127.0.0.1", 1885)
        .client_id("dummy")
...
async fn some_outgoing_and_no_incoming_should_trigger_pings_on_time() {
    let keep_alive = 5;
    let options = OptionsBuilder::new_tcp("127.0.0.1", 1886)
        .client_id("dummy")
        .keep_alive(Duration::from_secs(keep_alive))
        .finalize();
...
async fn some_incoming_and_no_outgoing_should_trigger_pings_on_time() {
    let keep_alive = 5;
    let options = OptionsBuilder::new_tcp("127.0.0.1", 2000)
        .client_id("dummy")
        .keep_alive(Duration::from_secs(keep_alive))
        .finalize();
...
async fn detects_halfopen_connections_in_the_second_ping_request() {
    let options = OptionsBuilder::new_tcp("127.0.0.1", 2001)
        .client_id("dummy")
...
async fn requests_are_blocked_after_max_inflight_queue_size() {
    let options = OptionsBuilder::new_tcp("127.0.0.1", 1887)
        .client_id("dummy")
        .outgoing_inflight(std::num::NonZero::new(5).unwrap())
        .finalize();
...
async fn requests_are_recovered_after_inflight_queue_size_falls_below_max() {
    let options = OptionsBuilder::new_tcp("127.0.0.1", 1888)
        .client_id("dummy")
        .outgoing_inflight(std::num::NonZero::new(3).unwrap())
        .finalize();
...
async fn packet_id_collisions_are_detected_and_flow_control_is_applied() {
    let options = OptionsBuilder::new_tcp("127.0.0.1", 1891)
        .client_id("dummy")
        .outgoing_inflight(std::num::NonZero::new(10).unwrap())
        .finalize();
...
async fn next_poll_after_connect_failure_reconnects() {
    ...
    let options = OptionsBuilder::new_tcp("127.0.0.1", 3000)
        .client_id("dummy")
        .finalize();
...
async fn reconnection_resumes_from_the_previous_state() {
    let options = OptionsBuilder::new_tcp("127.0.0.1", 3001)
        .client_id("dummy")
        .keep_alive(Duration::from_secs(5))
        .clean_start(false)
        .finalize();
...
async fn reconnection_resends_unacked_packets_from_the_previous_connection_first() {
    let options = OptionsBuilder::new_tcp("127.0.0.1", 3002)
        .client_id("dummy")
        .keep_alive(Duration::from_secs(5))
        .clean_start(false)
        .finalize();
...
async fn state_is_being_cleaned_properly_and_pending_request_calculated_properly() {
    let options = OptionsBuilder::new_tcp("127.0.0.1", 3004)
        .client_id("dummy")
        .keep_alive(Duration::from_secs(5))
