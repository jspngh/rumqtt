...
//!
//! This module is called `v4` because that is the protocol version of MQTT 3.1.1
//!
use bytes::BytesMut;
...
impl Protocol for V4 {
    ...
    fn read(stream: &mut BytesMut, max_size: usize) -> Result<Self::Item, Error> {
    ...
    fn write(packet: Self::Item, stream: &mut BytesMut, max_size: usize) -> Result<usize, Error> {
        let size = size(&packet)?;
        if size > max_size {
            return Err(Error::OutgoingPacketTooLarge {
                pkt_size: size as u32, // TODO
                max: max_size as u32,  // TODO
            });
        }
...
fn size(packet: &Packet) -> Result<usize, Error> {
